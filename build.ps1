...
# --- 3. Strip every “additionalProperties”: false ----------------------------
$spec = Get-Content '.\spam-api-swagger.json' -Raw

# ① remove  `"additionalProperties": false`  (with optional leading comma / whitespace)
$spec = $spec -creplace '(?m),?\s*"additionalProperties"\s*:\s*false\b', ''

# ② tidy up dangling commas before } or ]
$spec = $spec -creplace ',(\s*[\}\]])', '$1'

# write back
Set-Content '.\spam-api-swagger.json' $spec
...
(Get-Content '.\client\src\Cloudmersive.APIClient.NET.Spam\packages.config').replace('<package id="RestSharp" version="105.1.0" targetFramework="net45" developmentDependency="true" />', '<package id="RestSharp" version="106.6.10" targetFramework="net45" developmentDependency="true" />') | Set-Content '.\client\src\Cloudmersive.APIClient.NET.Spam\packages.config'
(Get-Content '.\client\src\Cloudmersive.APIClient.NET.Spam\Client\ApiClient.cs').replace('request.AddFile(param.Value.Name, param.Value.Writer, param.Value.FileName, param.Value.ContentType);', 'request.AddFile(param.Value.Name, param.Value.Writer, param.Value.FileName, param.Value.ContentLength, param.Value.ContentType);') | Set-Content '.\client\src\Cloudmersive.APIClient.NET.Spam\Client\ApiClient.cs'
...
(Get-Content ./client/README.md).replace(' automatically generated by the [Swagger Codegen](https://github.com/swagger-api/swagger-codegen) project', " for the [Cloudmersive Spam Detection API](https://www.cloudmersive.com/spam-detection-api)") | Set-Content ./client/README.md
